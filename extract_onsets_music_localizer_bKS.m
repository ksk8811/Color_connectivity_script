...
allSubs = allSubs(:,[1 3 5 7 2 4 6 8]);% rearrange columns
...
for s = 1:length(unique(allSubs.Subject))
    ...
    names={'numbers','wpairs','faces','houses','tools','body','buttonpress'};
    
    onsets_mat_file='onsets_words'; % nom de la matrice d'onsets
    ...
    onsets{6}=allonsets(find(types==6));
    onsets{7}=b;
