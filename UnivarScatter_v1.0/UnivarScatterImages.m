function [xPositions, yPositions, Label, RangeCut] = UnivarScatter(data, images, im_names, varargin)
%--------------------------------------------------------------------------
...
for i=1:size(data,2)
    ...
    %% added a part with images. maybe will work withput the scatter, this is nasty. bad. awfull. but works
    ...
    im_names.Var1=double(im_names.Var1);
    
    
    to_plot = images(im_names.Var1==i);
    to_plot = to_plot(y_SortingIndex);
    
    for u = 1:length(to_plot)
        
        im = to_plot{u};
        x_val = 0.08;
        y_val = 1.82;
        %im_res = imresize(im, [50 50]);
        ...
        imagesc([xValues(u)-x_val,xValues(u)+x_val], [yValues(u)-y_val,yValues(u)+y_val], im)
        
    end
    
    
    %If we mariawant lines to represent the SEM and the std
