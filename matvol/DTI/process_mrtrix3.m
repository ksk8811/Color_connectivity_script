...
for nbsuj = 1:length(V4D)
    ...
    if ~skip
        ...
        cmd = sprintf('%s;\n dwi2response tournier %s response.txt -grad %s -mask %s -voxels sf.nii.gz -nthreads %d', cmd, the4D,  par.grad_file, par.sf_mask, par.nthreads);
