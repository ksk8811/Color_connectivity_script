...
if iscell(mag)
    for nbs=1:length(mag)
        ...
        %try
            script_QSM_SPM(seq,mag{nbs},phase{nbs},outdir{nbs});
        %catch
        %    fprintf('\n\nERROR for suj %s\n',outdir{nbs});
        %    keyboard
        %end
...
ind = isnan(TE);
if any(ind)
    TE(find(ind)) = (TE(find(ind)-1) + TE(find(ind)+1))/2
end
...
%% QSM SDI + TSVD /// Fast QSM and TSVD + MEDI L1
laplacian_phase=LW(phase_combine).*new_mask;
phase_TSVD=ifftn(fftn(laplacian_phase).*inv_kernel_fourier_tsvd).*Mask; % laplacian and kernel taken from SDI
qsm_SDI=ifftn(fftn(laplacian_phase).*K);
qsm_SDI=qsm_SDI.*Mask; % Nicer

%%%%Save results on NIFTI Format
%Nom=header_spm.fname;
%Nom=[num2str(seq) Nom];
%Nom=Nom(1:end-4);
%mkdir(Nom);
header_spm.descrip = 'QSM SDI (ppm)';
header_spm.fname = ['qsm_SDI.img'];
header_spm.dt=[16 0];
spm_write_vol(header_spm, qsm_SDI);
%%%%

qsm_TSVD=MEDI_L1(lambda,phase_TSVD,N_std,Mask,iMag,matrix_size,voxel_size,B0_dir,Tau);

%%%%Save results on NIFTI Format
header_spm.descrip = 'QSM TSVD (ppm)';
header_spm.fname = ['qsm_TSVD.img'];
spm_write_vol(header_spm, qsm_TSVD);
%%%%

%% QSM SHARP /// SHARP + MEDI L1
[SHARP_phase]=SHARP(true_phase,matrix_size,voxel_size,Mask);
qsm_SHARP=MEDI_L1(lambda,SHARP_phase,N_std,Mask,iMag,matrix_size,voxel_size,B0_dir,Tau);

%%%%Save results on NIFTI Format
header_spm.descrip = 'QSM SHARP (ppm)';
header_spm.fname = ['qsm_SHARP.img'];
spm_write_vol(header_spm, qsm_SHARP);

if exist('lbv_phase.mat')
    %% QSM LBV /// LBV + MEDI L1
