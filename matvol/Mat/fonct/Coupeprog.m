...
% Modif VP %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     set(gca,'Xlim',[0 abs(dim(1))]);   set(gca,'Ylim',[0 abs(dim(2))])
     fov = abs(vox.* dim);
     AspectRatio = fov(1)/fov(2);
     set(gca,'PlotBoxAspectRatio', [AspectRatio 1 1])
     pos_axe  = get(gca,'position'); 
     max_axes = hdl.view.max;
     hdla.cur_pos(3:4) = max_axes(1:2);
     pos_axe(3:4) = max_axes(1:2);
     set(AxeNum,'position',pos_axe,'visible','on');  
     set(gca,'PlotBoxAspectRatio', [AspectRatio 1 1])
     Axeshdl{hdla_num} = hdla; AxeshdlChanged
% Fin Modif VP %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
