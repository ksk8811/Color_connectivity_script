...
defaults.modality='FMRI';
     ...
     minimum=0; % 0 all fonction (even buggi one) 1 just the robustest 
...
% Modif VP %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
max_axes = [0.76 0.73];
hdl.axe(1) = axes('unit','normalized','Position',[0.0350    0.1129    0.3825    0.3657]);
hdl.axe(2) = axes('unit','normalized','Position',...
		  [0.4363    0.1129    0.3825    0.3657]);
hdl.axe(3) = axes('unit','normalized','Position',...
		  [0.0350    0.5000    0.3825    0.3657]);
hdl.axe(4) = axes('unit','normalized','Position',...
		  [0.4363    0.5000    0.3825    0.3657]);
 % Fin Modif VP %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
...
hdl.space.start = uicontrol( ...
        ...
        'String','start',...
        'ButtonDownFcn','affichevol(1,''startmoins'')',...
...
hdl.space.stop = uicontrol( ...
    ...
    hdl.space.pause = uicontrol( ...
        ...
        'BackgroundColor',[1 1 1],...
        'TooltipString','pause (in sec) for start',...
        'String','0');

if ~minimum

hdl.space.project = uicontrol('Parent',h0, ...
	'Units','normalized', ...
	'ListboxTop',0, ...
	'Position',[0.97 0.85 0.03 0.03], ...
        'TooltipString','3D project',...
	'Style','checkbox',...
	'Userdata',1,...
        'CallBack','affichevol(1,''set_project_scale'')');
    
    
   hdl.space.txt_nbvol = uicontrol( ...
	'Units','normalized', ...
	'ListboxTop',1, ...
	'Position',[0.91 0.81 0.061 0.03], ...
	'HorizontalAlignment','left',...
	'Style','text', ...
	'string', str,...
	'HorizontalAlignment', 'center',...
 ...
 hdl.space.time_slider = uicontrol(...             
	...
	'ListboxTop',0, ...
	'Position',[0.85 0.795 0.14 0.015], ...
	'Style','slider', ...
        'Tag','time_slider', ...
...
hdl.vol_list	 = uicontrol('Parent',figNum, ...
	...
	'Tag','ListCon', ...
	'HorizontalAlignment','left',...
        'TooltipString','choose your volume',...
	'Value',1);
...
hdl.color.edit_min = uicontrol('Parent',h0, ...
	...
	'TooltipString','min of the colormap',...
	'Style','text', ...
	'Tag','EditText1');
...
hdl.color.edit_max = uicontrol('Parent',h0, ...
	...
	'BackgroundColor',[0.7 0.7 0.7], ...
	'ListboxTop',0, ...
	'TooltipString','max of the colormap',...
...
%%%%%%%%%%%    UIMENU      %%%%%%%%%%%%%%%%%%%%%%%%    
    ...
    uimenu(menuh1,'label','save all','callback', 'affichevol(1,''saveall'')');
    uimenu(menuh1,'label','load all','callback', 'affichevol(1,''loadall'')');
    ...
    uimenu(menuh3,'label','expand slice roi','callback','affichevol(3,''expandROI'')');

    uimenu(menuh3,'label','load ROI','callback','affichevol(3,''loadPos'')');

  mars_build =  uimenu(menuh3,'label','build ROI');
   choi = {'blob','image','sphere','box'};
   for kk=1:length(choi)
     uimenu(mars_build,'label',choi{kk},'callback','affichevol(3,''buildROI'')');
   end
    uimenu(mars_build,'label','Equerre','callback','affichevol(3,''Equerre'')');
    uimenu(mars_build,'label','Equerre distance','callback','affichevol(3,''Equ_set_d'')','userdata',40);

    uimenu(mars_build,'label','3D merge','callback','affichevol(3,''3D merge'')');

 uimenu(menuh3,'label','Save ROI','callback','affichevol(3,''SavePos'')');
 uimenu(menuh3,'label','Save as vol noise','callback','affichevol(3,''SavePos_noise'')');
%uimenu(menuh3,'label','ROI->mask','callback','affichevol(3,''ROItomask'')');
 uimenu(menuh3,'label','clear ROI','callback','affichevol(3,''CleanPos'')');
    ...
    visu    = uimenu(menuh3,'label','color or size');
    ...
    uimenu(menuh4,'label','new Contrast','callback','affichevol(4,''New_Con'')');
...
%    uimenu(menuh4,'label','Seuil','callback','affichevol(4,''Seuil'')','enable','off');
    ...
    uimenu(menuh5,'label','roi','callback','affichevol(5,''charge_roi'')');
   ...
   menuh6 = uimenu(gcf,'label','graph');
    ...
    uimenu(menuh6,'label','navigate','callback','affichevol(1,''navigate'')');
    uimenu(menuh6,'label','pixval','callback','affichevol(1,''pixval'')');
%    uimenu(menuh6,'label','axeson','callback','set(gca,''visible'',''on'')');
    ...
    uimenu(menuh6,'label','hide','callback','affichevol(11,''hide_all'')');
...
menu_view = uimenu(gcf,'label','view');
  uimenu(menu_view,'label','1 views','Callback','affichevol(1,''singleV'')');
  uimenu(menu_view,'label','4 views','Callback','affichevol(1,''multiV4'')');
if ~minimum
  uimenu(menu_view,'label','3D view','Callback','affichevol(1,''3D view'')');
  uimenu(menu_view,'label','choose vol','Callback','affichevol(1,''chooseVol'')');
end
  uimenu(menu_view,'label','print info','callback', ...
       'affichevol(1,''print_orient_info'')');
...
menu_load = uimenu(gcf,'label','load');
  uimenu(menu_load,'label','Data','Callback','affichevol([''Data'' ] )');
hr=  uimenu(menu_load,'label','Result','Callback','affichevol([''Result'' ] )');
  uimenu(menu_load,'label','Save','Callback','affichevol(1,''SaveData'' )');

  uimenu(menu_load,'label','free_surf','Callback','affichevol(1,''load_free_s'')');
if minimum
    uimenu(menu_load,'label','volume calc','callback',...
	   'affichevol(6,''volumes_expr'')');
    set(hr,'visible','off')
else
    menucalc = uimenu(menu_load,'label','image_calc');
    uimenu(menucalc,'label','from_volume_list','callback',...
	   'affichevol(6,''volumes_expr'')');
    uimenu(menucalc,'label','remove model','callback',...
	   'affichevol(6,''SPM_expr'')');
end
...
%  hdla.Im =surf(V(:,:,1),'EraseMode','none');
...
%  set(gca,'ydir','normal','xdir','reverse')
  ...
  Vr.coupe=1; Vr.M_rot = AxialMat; Vr.num=1; Vr.numrot=1;
  hdla.Vr = Vr;
