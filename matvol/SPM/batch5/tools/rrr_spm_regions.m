function [Y,xY,voiname] = spm_regions(xSPM,SPM,hReg,xY)
...
% Karl Friston
% $Id: spm_regions.m 3354 2009-09-03 15:25:12Z guillaume $


% get figure handles
%--------------------------------------------------------------------------
%rrr no graphics Finter = spm_figure('FindWin','Interactive');
%rrr no graphics Fgraph = spm_figure('GetWin','Graphics');
%rrr no graphics header = get(Finter,'Name');
%rrr no graphics set(Finter,'Name','VOI time-series extraction')
try
    xY;
catch
    xY = {};
...
%--------------------------------------------------------------------------
%rrr no graphics spm_XYZreg('SetCoords',xyz,hReg);


%-Get adjustment options and VOI name
%--------------------------------------------------------------------------
%rrr no graphics spm_input(sprintf('at [%3.0f %3.0f %3.0f]',xY.xyz),1,'d','VOI time-series extraction')
...
% voxels defined
%--------------------------------------------------------------------------
%rrr no graphics  spm('Pointer','Watch')
...
%-Reset title
%--------------------------------------------------------------------------
%rrr no graphics spm('FigName',header);
%rrr no graphics spm('Pointer','Arrow')
