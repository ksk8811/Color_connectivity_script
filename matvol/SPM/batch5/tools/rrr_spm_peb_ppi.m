function PPI = spm_peb_ppi(SPM,voiname,contrast_to_include,namePPI)
...
%rrr do not aske i          = spm_input('Analysis type?',1,'m',ppiflag);
i=2;
ppiflag    = ppiflag{i};
...
%-------------------------------------------------------------------------
PPI.name    = namePPI; % spm_input('Name of PPI',3,'s','PPI');
...
str     = ['PPI_' PPI.name];
str2 = [str,'_user_reg'];
R=[PPI.Y,PPI.P,PPI.ppi];

if spm_matlab_version_chk('7') >= 0,
    save(fullfile(SPM.swd,str),'-V6','PPI')
    save(fullfile(SPM.swd,str2),'-V6','R')
else
    save(fullfile(SPM.swd,str),'PPI')
    save(fullfile(SPM.swd,str2),'R')
end
