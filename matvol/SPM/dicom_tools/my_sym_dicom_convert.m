function fnames=spm_dicom_convert(hdr,opts,root_dir,format)
...
if strcmp(root_dir, 'flat2')
    % rrr naming convention
    %-------------------------------------------------------------------
    ...
    serdes = strrep(strip_unwanted(hdr.SeriesDescription),strip_unwanted(hdr.ProtocolName),'');

    fname = sprintf('%s_E%s_%s%s_%.4d.%s',strip_unwanted(hdr.PatientsName),strip_unwanted(hdr.StudyID),strip_unwanted(hdr.ProtocolName),serdes, hdr.SeriesNumber,format);
