function [indV indS] = do_art_slice(Pimages,OUTSLICE,verbose)
...
%     using linear interpolation of the before and after volumes.fmidel_right,fup_right,
...
%  end
if ~exist('Pimages'),   Pimages = get_subdir_regex_files();end
if ~exist('OUTSLICE'),  OUTSLICE = 5;end
if ~exist('verbose');   verbose=0;end


if iscell(Pimages)
    for nbsuj=1:length(Pimages)
        do_art_slice(char(Pimages(nbsuj)));
    end
    return
end
...
% fprintf('\n NEW IMAGE FILES WILL NOT BE CREATED');
...
mask_flag = 1;
    ...
    if mask_flag == 2
        ...
        %fprintf('\n');
        Pnames = P(1).fname;
        Automask = art_automask([Pnames(1,:) ',1'],-1,1);
        maskcount = sum(sum(sum(Automask)));  %  Number of voxels in mask.
        voxelcount = prod(size(Automask));    %  Number of voxels in 3D volume.
    end
end
...
if slicefilt == 1  % Prepare some thresholds for slice testing.
    ...
    orient = 0;
    if ( vx < vy & vx < vz ) orient = 1; if verbose, disp(' Remove bad Sagittal slices'); end;end
    if ( vy < vx & vy < vz ) orient = 2; if verbose, disp(' Remove bad Coronal slices'); end;end
    if ( vz < vx & vz < vy ) orient = 3; if verbose, disp(' Remove bad Axial slices'); end;end
    nslice = min([vx vy vz]);
    if ( orient == 0 ) 
    ...
    p = zeros(min(10,length(P)),nslice);%p = zeros(3,nslice);
    for i = 1:min(10,length(P))  %3
...
%     OUTSLICE = spm_input(['Select threshold (default is shown)' ],1,'n',OUTSLICEdef); 
    ...
    if verbose, disp('Writing Artifact Log to location of image set');end
    ...
    if verbose
        ...
        fprintf(logID,'\n  Outslice (counts) = %4.1f \n Outslice sugested = %4.1f' , OUTSLICE,percentbad);
