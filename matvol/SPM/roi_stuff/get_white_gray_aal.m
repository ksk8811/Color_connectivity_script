...
for k=1:length(VY)
  ...
  for ka=1:size(roiaal,1)
    ...
    if ~is_empty_roi(oaw) && ~is_empty_roi(oag)
      if k==1
	Yaal_g(ka) = get_roi_data(oag,VY(k),'',k);
	Yaal_w(ka) = get_roi_data(oaw,VY(k),'',k);
      else
	Yaal_g(ka) = get_roi_data(oag,VY(k),Yaal_g(ka),k);
	Yaal_w(ka) = get_roi_data(oaw,VY(k),Yaal_w(ka),k);
      end
    end
